...
export async function runExport(options: RunExportOptions): Promise<void> {
  const { jobId, userId, dateFrom, dateTo, includeOnline, triggerType } = options;
  ...
  try {
    ...
    // 2. Fetch transactions for all stores
    ...
    // 6. Upload to S3
    const txFileName = `transactions_${dateFrom}_to_${dateTo}_${timestamp}_${randomSuffix()}.csv`;
    const invFileName = `inventory_${dateFrom}_to_${dateTo}_${timestamp}_${randomSuffix()}.csv`;
    const salesFileName = `sales_summary_${dateFrom}_to_${dateTo}_${timestamp}_${randomSuffix()}.csv`;
    ...
    console.log(`[Export ${jobId}] Completed successfully. ${allTransactions.length} transactions, ${allInventory.length} inventory items, ${salesSummaryRows.length} products in sales summary across ${stores.length} stores.`);

    // 9. Notify owner
    const completedAt = formatDateTimeBangkok(new Date());
    await notifyOwner({
      title: `StoreHub Export Completed (${triggerType})`,
      content: `Export job #${jobId} completed successfully at ${completedAt} (GMT+7).\n\n` +
        `Date range: ${dateFrom} to ${dateTo}\n` +
        `Stores: ${stores.length}\n` +
        `Transactions: ${allTransactions.length}\n` +
        `Inventory items: ${allInventory.length}\n` +
        `Sales summary products: ${salesSummaryRows.length}\n\n` +
        `Files:\n- ${txFileName}\n- ${invFileName}\n- ${salesFileName}`,
    });
  } catch (err) {
